...
Hitch turns Codex, Claude Code, Hermes, Pi, OMP, OpenCode, and Antigravity hooks into one portable event protocol. Write a hook once, run it across harnesses, preserve the native payload, and route lifecycle events to local or remote handlers.
...
- Codex, Claude Code, Hermes, and Antigravity shell hook shim entrypoints via `hitch-client`.
- Claude Code hook installation into `~/.claude/settings.json`.
...
- Harness detection for Codex, Claude Code, Hermes, Pi, OMP, OpenCode, and Antigravity.

The source installer creates missing Hitch user config with `hitch config init`, prompts for a Hitch server URL through `/dev/tty` even when installed with `curl ... | sh`, and can persist `HITCH_URL` for remote servers. `hitch-client install` installs supported Codex, Claude Code, and Hermes hooks, installs managed Pi and OMP extensions, installs a managed OpenCode plugin, and prefers `hitch-client` in managed shell-hook commands when it is installed beside `hitch`. Generated hooks resolve the server URL at runtime unless `--url` pins one.
