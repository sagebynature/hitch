...
func plannedOps(harnesses []string, uninstall bool, apiURL string) ([]installOperation, error) {
	...
	for _, h := range harnesses {
		...
		case "pi":
			...
			ops = append(ops, installOperation{Harness: h, Action: action, Path: detection.ConfigPath, BackupPath: timestampedBackupPath(h, filepath.Base(detection.ConfigPath)), Status: "planned", Reason: apiURL})
		}
	}
	return ops, nil
}
...
func applyOps(ops []installOperation, uninstall bool) error {
	for _, op := range ops {
		...
		case "install_pi_extension":
			if err := installPiExtension(op.Path, op.BackupPath, op.Reason); err != nil {
				return err
			}
...
func piExtensionContent(apiURL string) ([]byte, error) {
	...
	events := make([]string, 0, len(piExtensionEvents))
	for _, event := range piExtensionEvents {
...
async function dispatchToHitch(sourceEventType, event, ctx) {
  ...
  try {
    const response = await fetch(HITCH_API_URL + "/v1/dispatch-sync", {
      method: "POST",
