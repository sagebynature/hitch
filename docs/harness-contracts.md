...
- Pi: maps extension callback events in Go and installs a managed TypeScript extension that posts callbacks to `/v1/dispatch-sync`, applies `adapter_action` return values or mutations, and fails open when Hitch is unavailable. OMP shares the Pi adapter-response contract in Go, but its installer patcher is still unsupported.
