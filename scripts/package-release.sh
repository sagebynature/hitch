...
for target in $platforms; do
  ...
  hitch_binary="$build_dir/hitch"
  client_binary="$build_dir/hitch-client"
  if [ "$goos" = windows ]; then
    hitch_binary="$hitch_binary.exe"
    client_binary="$client_binary.exe"
  fi

  printf 'building %s/%s...\n' "$goos" "$goarch"
  CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" go build -trimpath -ldflags "$ldflags" -o "$hitch_binary" ./cmd/hitch
  CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" go build -trimpath -ldflags "$ldflags" -o "$client_binary" ./cmd/hitch-client
...
for build_dir in sorted(p for p in build_root.iterdir() if p.is_dir()):
    ...
    hitch_binary = build_dir / ("hitch.exe" if is_windows else "hitch")
    client_binary = build_dir / ("hitch-client.exe" if is_windows else "hitch-client")
    for binary in (hitch_binary, client_binary):
